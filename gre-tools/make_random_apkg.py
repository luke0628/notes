#!/usr/bin/env python3
"""Generate 10 random GRE words .apkg with bold + sound + 护眼绿"""

import json, re, genanki, os, zipfile

# --- Read refined data ---
with open('/home/Lu/gre_sample_random10_refined.json') as f:
    words = [json.loads(line) for line in f if line.strip()]

with open('/home/Lu/gre_sample_random10_raw.json') as f:
    raw_list = json.load(f)
raw_map = {}
for r in raw_list:
    raw_map[r['name'].lower()] = r

# --- Extract available sound files from source apkg ---
apkg_path = '/home/Lu/Documents/GRE_3000_BrillliantZ.apkg'
snd_dir = '/home/Lu/gre_sounds_random'
    ...
    for r in raw_list:
        ...
        if m:
            ...
            for num, fname in num_to_name.items():
                ...
                print(f"  {name}: sound not in media ({snd_name})")
        else:
...
# --- Build deck ---
deck = genanki.Deck(2025062711, 'GRE精修3000-随机样本')

# For the 7 words without media, we just skip them (Anki will look for them in collection.media)
...
out_path = '/home/Lu/Documents/gre_sample_random10.apkg'
